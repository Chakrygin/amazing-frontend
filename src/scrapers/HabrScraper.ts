...
export default class HabrScraper implements Scraper {
  ...
  private readonly hub: Link = {
    title: Hubs[this.id],
    href: `https://habr.com/ru/hub/${this.id}/`,
  };

  async *scrape(): AsyncGenerator<Post> {
    core.info(`Parsing html page by url '${this.hub.href}'...`);

    const response = await axios.get(this.hub.href);
